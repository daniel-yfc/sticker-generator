 ...
 * Accepts an optional runtime key (e.g. from user input) and falls back
 * to the environment variable if none is provided.
 *
 * @param apiKey - Optional runtime API key supplied by the caller
 * @throws Error if no valid key is found
 */
function validateApiKey(apiKey?: string): string {
  const activeApiKey = apiKey || process.env.API_KEY;

  if (!activeApiKey || typeof activeApiKey !== 'string' || activeApiKey.trim().length === 0) {
 ...
 * @param apiKey - Optional runtime API key; falls back to process.env.API_KEY
...
export const generateSticker = async (
  ...
  // Validate API key — accepts optional runtime key, falls back to env
  const activeApiKey = validateApiKey(apiKey);
...
export const generateStickerSet = async (
  ...
  return throttledMap(
    variations,
    (v) => generateSticker(sourceImageBase64, style, v, apiKey),
    BATCH_CONCURRENCY
  );
};
