...
*   **擴展套裝貼圖生成：** 能夠從單張照片生成一組 3 張擴充貼圖，保持風格一致但展現不同表情動作（例如：豎起大拇指、大笑、驚訝）。
...
3.  **設定環境變數 (`.env.local`)：**
    ...
    ```
    *(備註：`vite.config.ts` 會將 `GEMINI_API_KEY` 映射到應用程式使用的 `process.env.API_KEY` 中)。*
