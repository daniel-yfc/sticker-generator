...
const App: React.FC = () => {
  ...
  const isStickerSetVisible =
    (status === AppStatus.SET_SUCCESS || status === AppStatus.SET_PARTIAL) &&
          ...
          <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-[600px]">
            ...
            <div className="flex-1 bg-gray-100 rounded-3xl border border-gray-200 shadow-inner overflow-hidden relative flex flex-col">
              ...
              <div className="flex-1 flex items-center justify-center p-4 lg:p-8 overflow-auto">
                ...
                {/* SET_SUCCESS and SET_PARTIAL both render StickerSetView with per-tile state */}
                {isStickerSetVisible && (
                  <StickerSetView
                    tiles={generatedTiles}
                    style={selectedStyle}
                    onReset={handleReset}
                    onRetryTile={(variationId) => retryStickerSetTile(variationId as VariationId)}
                    t={t}
                    stylesTranslation={TRANSLATIONS[language].styles}
                  />
                )}
